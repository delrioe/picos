...
ruleset manage_sensors{
  ...
  meta{
    ...
    shares __testing, showChildren, sensors, temperatures, testFunc, findRxFromSubscription, testingFunction, reports
  ...
  global{
    ...
    findSubscriptionName = function (eci){
      sensorName = subscription:established("Tx", eci).head(){"Somethinghere"};
      sensrName
    }
    
    testingFunction = function( report ){
      ent:reportList{reportID}{"temperatures"}
    }
    
    reports = function ( reportID ){
      // x = (rerportID.isnull()) => "asdf" | 0;
      // ent:reportList.
      x
    }
    ...
    __testing = {"queries": [{"name": "showChildren"},
                              ...
                              {"name": "testingFunction", "args":["report"]},
                              {"name": "reports", "args":["reportID"]}
                              ], 
                  "events":  [ { "domain": "sensor", "type": "new_sensor", "attrs": [ "sensor_id" ] },
                                ...
                                {"domain":"request", "type":"report"},
                                {"domain":"reset", "type":"reportIDNumber"}
                            ] 
...
//accepts any subscription
 ...
 rule autoAcceptSubscriptionNotSensor{
  ...
  rule requestReport{
    ...
    foreach sensors().map(function(x) {x}) setting(sub)
    pre {
      id = ent:reportID ||  random:uuid()
    }
      event:send(
        {"eci":sub{"Tx"}, "domain":"request", "type":"report", 
      "attrs":{
        "reportID": id,
        "requester": findRxFromSubscription(sub{"Tx"})
        }}
      )
      ...
      always{
        ...
        raise create event "report" attributes {
          "times" : numSensors,
          "reportID": id
        } on final;
        clear ent:reportID on final;
        
      } 
  }
  
  rule createReport{
    select when create report
    pre{
      times = event:attr("times") 
      report = event:attr("reportID")
    }
      
    always{
        // ent:reportID := ent:reportID.defaultsTo(0) + 1;
        ent:reportList := ent:reportList.defaultsTo({}).put(report, { 
          "temperature_sensors" : times, 
          "responses": 0, 
          "temperatures": []});
  ...
  rule receiveReport{
    select when received report
    pre{
      reportID = event:attr("reportID").klog("ReportID: ")
      temperatures = event:attr("temperatures").klog("Temperatures: ")
      sender = event:attr("sender").klog("Eci of sender: ")
      nameSender = event:attr("nameSender").klog("Name Sender: ")
    }
    send_directive("report completed")
    always{
      // ent:reportList := ent:reportList.put{[reportID, "temperatures"]}.put("Name_of_sensor", ) 
      //Todo
      ent:responses := ent:responses.defaultsTo(0);
      ent:responses := ent:responses + 1;
      sensorToAdd = {}.put(nameSender, temperatures);
      sensorToAdd.klog("Sensor to add before");
      ent:reportList := ent:reportList.set([reportID, "temperatures"], ent:reportList{reportID}{"temperatures"}.append(sensorToAdd));
      ent:reportList := ent:reportList.set([reportID, "responses"], ent:responses);
    }
  }



  rule resetReportIDNumber{
    select when reset reportIDNumber
    always{
      ent:reportID := 0
    }
  }
